...
\begin{itemize}
    ...
    \item \textbf{Variables}: $p = 12$ quantitative environmental and sustainability metrics
...
\begin{enumerate}
    \item \textbf{Kaiser Criterion}: Retain components with $\lambda_k > 1$
...
We selected 12 quantitative variables for PCA:
...
Table \ref{tab:eigenvalues} presents the eigenvalue decomposition results.
...
\midrule
PC1 & 1.1041 & 9.20 & 9.20 \\
PC2 & 1.0596 & 8.83 & 18.03 \\
PC3 & 1.0549 & 8.79 & 26.81 \\
PC4 & 1.0455 & 8.71 & 35.52 \\
PC5 & 1.0335 & 8.61 & 44.13 \\
PC6 & 1.0058 & 8.38 & 52.51 \\
\midrule
PC7 & 0.9833 & 8.19 & 60.70 \\
PC8 & 0.9762 & 8.13 & 68.84 \\
PC9 & 0.9655 & 8.04 & 76.88 \\
PC10 & 0.9465 & 7.88 & 84.76 \\
PC11 & 0.9299 & 7.75 & 92.51 \\
PC12 & 0.8991 & 7.49 & 100.00 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Observation:} The eigenvalues are relatively uniform (ranging from 0.90 to 1.10), indicating low correlation among variables. This is characteristic of near-orthogonal variable structure.

\subsection{Component Selection}

Using the Kaiser criterion ($\lambda > 1$), we retain \textbf{6 principal components}, explaining 52.51\% of total variance.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figures/scree_plot.png}
    \caption{Scree plot showing eigenvalues (left) and explained variance (right). The red dashed line indicates the Kaiser criterion threshold ($\lambda = 1$).}
    \label{fig:scree}
\end{figure}
...
\textbf{PC1 (9.20\% variance):} Captures the primary environmental impact dimension, with loadings on carbon emissions, water usage, and production volume.

\textbf{PC2 (8.83\% variance):} Represents the sustainability governance dimension, including transparency and compliance scores.

\textbf{PC3-PC6:} Capture additional latent factors related to economic performance, ethical ratings, and regional variations.
...
\begin{equation}
    Q_{1,2} = \tau_1 + \tau_2 = 9.20\% + 8.83\% = 18.03\%
\end{equation}

This relatively low value indicates that the original variables are largely independent, which is actually valuable information about the data structure.
...
\begin{enumerate}
    \item \textbf{Dimensionality Reduction:} 12 variables reduced to 6 principal components retaining 52.51\% variance (Kaiser criterion)
    
    \item \textbf{Variable Independence:} The near-uniform eigenvalue distribution indicates low multicollinearity among environmental metrics, suggesting they capture distinct aspects of sustainability
    
    \item \textbf{Interpretation:} The first two components primarily capture environmental impact and governance dimensions respectively
    
    \item \textbf{Brand Analysis:} No significant clustering of brands suggests similar environmental profiles across the fast fashion industry
\end{enumerate}
